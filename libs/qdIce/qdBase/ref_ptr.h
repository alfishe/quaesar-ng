...
#include "qdIce/qdPlatform/compiler.h"


#define SAFE_DELETE_REF_PTR(pPtr) \
	 { if ( pPtr != nullptr ) { pPtr->ref_ptr_release_and_delete(); pPtr = nullptr;}; }
...
namespace qd
{
	...
	template<typename T> class wref_ptr;
	...
	class RefCounted
	{
        ...
        template <class T>
		friend void Details::referenced_static_delete(T* pThis);

		template<typename T> friend class Details::ref_ptr_base;
		template<typename T> friend class Details::ref_ptr_getter;
		template<typename T> friend class Details::wref_ptr_getter;
	...
	public:
		...
		inline bool is_() const {
			return dynamic_cast<const T2*>( /*(const T2* )*/this ) != 0 /*nullptr*/; // NOT WORKED IF COMPLEX DERIVITY HIERACTION
		}
#endif // EA_COMPILER_NO_RTTI
		...
		template<class T2>
		inline typename MP::InnerType<T2>::type* get_if_() const
		{
#if !defined EA_COMPILER_NO_RTTI
		...
		inline TRefInt ref_ptr_release() const {
			return _ref_ptr_release();
		}

		inline TRefInt ref_ptr_release_and_delete() /*const*/
		{
			if (!c_def(this)) return 0;
			return _ref_ptr_release_and_delete(this);
		}

		TRefInt ref_ptr_count() const {
			return _ref_ptr_RefCount;
		}

		TRefInt wref_ptr_count() const {
			return _ref_ptr_WeakRefCount;
		}

		inline bool is_ref_ptr_valid() const {
			return c_def(this) && this->_is_ref_ptr_valid();
		}
	...
	namespace Details
	{
		...
		public:
			...
			inline static T* get( const ref_ptr_base<T>* pRefPtr) {
				return pRefPtr->_ptr;
			}
			...
			inline static void set_raw(ref_ptr_base<T>* pRefPtr, T* pNewPtr) {
				pRefPtr->_ptr = pNewPtr;
			}
		...
		public:
			...
			inline static T* get( const ref_ptr_base<T>* pRefPtr) {
				return pRefPtr->valid() ? pRefPtr->_ptr : nullptr;
			}
		...
		class ref_ptr_base
		{
			friend class Details::ref_ptr_getter<T>;
			friend class Details::wref_ptr_getter<T>;
		protected:
		...
		public:
			...
			inline bool valid() const {
				qd::RefCounted* pPtr = (qd::RefCounted*)( this->_ptr );
		...
		template<typename T, typename TRefPtrGetter = Details::ref_ptr_getter<T> >
		...
		public:
			...
			inline const T& getcref() const {
				return static_cast<const T&>( *this->getsafe() );
			}
			...
			inline bool operator== (const std::nullptr_t p) const {
			...
			inline bool operator < (const ref_ptr_base2& rp) const {
				return (get() < rp.get());
			}
		...
		protected:
			inline void destroy() {
				TRefPtrGetter::destroy(this);
			}

			inline void assign(const T* pNewInst) {
				TRefPtrGetter::assign( this, const_cast<T*>(pNewInst) );
			}

			inline void reset(const T* pNewInst) {
				TRefPtrGetter::reset( this, const_cast<T*>(pNewInst) );
			}

			inline ~ref_ptr_base2() {
				TRefPtrGetter::destroy(this);
			}
	...
	class ref_ptr : public Details::ref_ptr_base2< /*T*/ typename std::remove_const<XT>::type , Details::ref_ptr_getter</*T*/ typename std::remove_const<XT>::type > >
	{
		typedef typename std::remove_const<XT>::type T;
		typedef ref_ptr<T> TThis;
		typedef Details::ref_ptr_base2<T, Details::ref_ptr_getter<T> > TSuper;
	...
	public:
        ...
        T* operator -> () const { assert(TSuper::valid() && "NULL POINTER EXCEPTION");
			 return this->_ptr; //TSuper::_get_raw();
		}

        T& operator * ()  { assert(TSuper::valid()); return *TSuper::get(); }

		const T& operator * () const { assert(TSuper::valid()); return *TSuper::get(); }
		...
		/*explicit*/ inline ref_ptr( T* p ) {
			TSuper::reset( p );
		}

		/*explicit*/ inline ref_ptr( const T* p ) {
			TSuper::reset( p );
		}
...
// 		}
		...
		inline ref_ptr(const qd::CNullPtr&) {
		}
		...
		template<class T2>
		inline ref_ptr(const qd::ptr<T2>& rp) {
		...
		template<class T2>
		inline const TThis& operator = (const qd::ptr<T2>& rp) {
			TSuper::assign( rp.gcc_template get_<T>() );
			return *this;
		}


		static inline ref_ptr<T> make() {
			return new T();
		}
	...
	//////////////////////////////////////////////////////////////////////////
	template<typename T>
	class wref_ptr : public Details::ref_ptr_base2< T, Details::wref_ptr_getter<T> >
	{
		typedef wref_ptr<T> TThis;
		typedef Details::ref_ptr_base2< T, Details::wref_ptr_getter<T> > TSuper;
	...
	public:
		...
		inline T& operator * ()  {
			assert(TSuper::valid()); return *TSuper::get();
		}
...
// 		}
		...
		inline wref_ptr(const ref_ptr<T>& rp) {
			TSuper::reset( rp._get_raw() );
		}

		inline wref_ptr(const wref_ptr<T>& rp) {
			TSuper::reset( rp.get() );
		}
		...
		template<class T2>
		inline wref_ptr(const qd::ptr<T2>& rp) {
			TSuper::reset( rp.gcc_template get_<T>() );
		}
...
// NULL
	...
	template<typename T>
	inline bool isPtrNull( const qd::ptr<T>& pPtr ) {
		return !pPtr.valid();
	}

	template<typename T>
	inline bool isPtrNull( const qd::ref_ptr<T>& pPtr ) {
		return !pPtr.valid();
	}
	...
	template<class T>
	class rptr : public T, public RefCounted
	{
	public:
		rptr()
		{}
