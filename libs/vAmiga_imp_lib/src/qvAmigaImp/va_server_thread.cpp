...
void VAmServerThread::onVAmigaThreadMain() {
  try
  {
    ...
    {
        logDbg("VAMIGA: Loading Kick.rom from '%s' ...", g_cfg_startup->kickRomPath.c_str());
        try
        {
            vamiga::RomFile rom{g_cfg_startup->kickRomPath.c_str()};            m_pVAmiga->mem.loadRom(rom);
        }
        catch (...)
        {            m_threadErrStr = logErr("VAmiga load kickRom:'%s' ERROR!", g_cfg_startup->kickRomPath.c_str())/*->ASSERT_DLG()*/->getLogStr();
            m_threadErr = 1;
    ...
    for (;;) {
      ...
      logErr("VAmiga threadInit exception: '%s'", ex.what()).ASSERT_DLG();
