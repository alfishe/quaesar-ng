...
inline float inSine(float p) { return sinf((p - 1.f) * M_2_PI) + 1.f; }

// Modeled after quarter-cycle of sine wave (different phase)
inline float outSine(float p) { return sinf(p * M_2_PI); }

// Modeled after half sine wave
inline float inOutSine(float p) { return 0.5f * (1.f - cosf(p * M_PI)); }
...
inline float inElastic(float p) { return sinf(13.f * M_2_PI * p) * powf(2.f, 10.f * (p - 1.f)); }

// Modeled after the damped sine wave y = sinf(-13pi/2*(x + 1))*powf(2, -10x) + 1
inline float outElastic(float p) { return sinf(-13.f * M_2_PI * (p + 1.f)) * powf(2.f, -10.f * p) + 1.f; }
...
inline float inOutElastic(float p)
{
  if (p < 0.5f)
  {
    return 0.5f * sinf(13.f * M_2_PI * (2.f * p)) * powf(2.f, 10.f * ((2.f * p) - 1.f));
  }
  else
  {
    return 0.5f * (sinf(-13.f * M_2_PI * ((2.f * p - 1.f) + 1.f)) * powf(2.f, -10.f * (2.f * p - 1.f)) + 2.f);
  }
}
...
inline float inBack(float p) { return p * p * p - p * sinf(p * M_PI); }
...
inline float cosineFull(float p) { return 0.5f - cosf(p * M_2_PI) * 0.5f; }

inline float inStep(float p) { return p > 0.5f ? 1.f : 0.f; }

inline float outStep(float p) { return p < 0.5f ? 1.f : 0.f; }

inline float blink(float p) { return (p < 0.1f) ? 10.0f * p : 1.0f - (p - 0.1f) / 0.9f; }
inline float doubleBlink(float p) { return 0.5f - 0.5f * cosf(4 * M_PI * p); }

inline float blinkSin(float p)
{
  float val = (p < 0.1f) ? 10.0f * p : 1.0f - (p - 0.1f) / 0.9f;
  if (p < 0.5f)
    val *= 0.75f + 0.25f * sinf((p * 80.0f - 0.5f) * M_PI);
  return val;
}
...
inline float blinkCos(float p)
{
  float val = (p < 0.1f) ? 10.0f * p : 1.0f - (p - 0.1f) / 0.9f;
  if (p < 0.5f)
    val *= 0.75f + 0.25f * cosf((p * 40.0f - 0.5f) * M_PI);
  return val;
}
