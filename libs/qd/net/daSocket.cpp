...
ref_ptr<MemData> DaSocket::AppSlotDataProc::_ReadChunkSync(float /*TimeOut*/, bool bOneChunkOnly)
{
...
void CFTSocket::_onDataBlockReadedLock(ref_ptr<qd::MemData> pReadBuf)
{
    if (m_pDataProc)
    {
        ...
        ETrisult res = m_pDataProc->OnDataBlockReceived(
            pReadBuf); // DaSocket::AppSlotDataProc::OnDataBlockReceived( Inout_ pReadBuf )
...
ETrisult DaSocket::AppSlotDataProc::OnDataBlockReceived(/*Inout*/ DaSocket::TBlockPtr& pInOutBuf) /* OVERRIDE */
{
    ...
    while (nCurFilePos < nInBufSize) // SPLIT ON CHUNKS
    {
        ...
        if (!m_pCurChunkBuffer)
        {
            ...
            if (!ar.loadChunkTryTest(ChunkBufRead) || ChunkBufRead.getVer() < FTCHUNK_SOCKET_PACKET_HEADER_VER)
            {
                ...
                log_debug(
                    "socket:%X: CFTSocket::OnDataBlockReceived() WARNING: ERROR: - Bad ChunkBufRead IDs - SKIP BUFFER",
                    ptr2DW(m_pSocket));
                pInOutBuf->setSize(0);
            ...
            if (vlog_socket >= 3)
                log_debug("socket:%X: CFTSocket::OnDataBlockReceived() - Creates new Chunk buffer size: %u",
                    ptr2DW(m_pSocket), m_CurChunkLen);
        }
        ...
        if (m_pCurChunkBuffer->getSize() < m_CurChunkLen)
        {
            ...
            // APPEND BUFFER
            if (vlog_socket >= 3)
                log_debug("socket:%X: Expand buffer for full chunk size from %i to %i", ptr2DW(m_pSocket),
                    m_pCurChunkBuffer->getSize(), m_CurChunkLen);
        ...
        if (m_pCurChunkBuffer->getSize() == m_CurChunkLen)
        {
            // FULL CHUNK RECEIVED
            if (vlog_socket >= 4)
                log_debug("Socket:%X: CFTSocket::OnDataBlockReceived() - Full chunk received: %u", ptr2DW(m_pSocket),
                    m_CurChunkLen);
    ...
    if (nCurFilePos != 0)
    {
        ...
        else
        {
            ...
            if (vlog_socket >= 1)
                log_debug("Socket:%X: Move Readed buffer to start from nCurFilePos: %u nRemSize: %u", ptr2DW(m_pSocket),
                    nCurFilePos, nRemSize);
        }
    }
...
bool CFTSocket::AsyncRead(ref_ptr<qd::MemData> pReadBuf /*= nullptr*/)
{
    ...
    if (!IsConnected())
    {
        ...
        // throw CSocketException( CSocketException::CONNECTION_LOST, "Socket:%X is closed - can't AsyncRead", ptr2DW(
        // this ) );
        return false;
    }
    ...
    if (pReadBuf)
    {
        ...
        if (vlog_socket >= 3)
            log_debug("Socket:%X START ASync Read - Pos=%u Capacity=%u bytes", ptr2DW(this), pReadBuf->getSize(),
                bufCapacity);
    }
    else
    {
...
{
    if (vlog_socket >= 0)
        log_debug("CFTSocket::OnSocketConnected to \"%s:%u\" (%s) socket:%X - Start ASYNC READ",
            CC(m_ConnectAddr.GetHost()), (uint32_t)m_ConnectAddr.m_Port, CC(m_ConnectName), ptr2DW(this));
...
ref_ptr<MemData> DaSocket::AppSlotDataProc::_PopReadOneBody()
{
    qd::MutexLock ml(m_OutBufMutex);
...
ref_ptr<qd::MemData> DaSocket::AppSlotDataProc::ReadCommands(const DaSocket::ReadCmd_t& ReadParam)
{
...
ref_ptr<qd::MemData> DaSocket::AppSlotDataProc::InsertPacketHeader(void* pBuf, uint32_t Size)
{
...
void CFtTcpListner::OnServerConnectionAcceptedCallback(ref_ptr<CFTSocket> pSocket, FTSocketErrorCode error)
{
    ...
    {
        if (vlog_socket >= 1)
            log_debug("CFtTcpListner::OnServerConnectionAcceptedCallback() - Error occured code:%i socket:%X",
                (int)error, ptr2DW(pSocket));
    }
...
{
    return new DaSocket::AppSlotDataProc();
}
...
CFtTcpListner::CFtTcpListner(CSocketService* pIOService, unsigned short nPort,
    const CFtTcpListner::TOnConnectCB& OnConnect, qd::string Name)
...
void CFtTcpListner::_listnerThreadProc()
{
    ...
    if (vlog_socket >= 1)
        log_debug("CFtTcpListner::_ListnerThreadProc() port:%i  Name:\"%s\" - START WAITING THREAD", (int)m_nListenPort,
            CC(m_Name));
    ...
    if (vlog_socket >= 1)
        log_debug("CFtTcpListner::_ListnerThreadProc() port:%i Name:\"%s\" WARNING: CANCEL LISTENING  - EXIT THREAD",
            (int)m_nListenPort, CC(m_Name));
...
void CFtTcpListner::_onStartConnectionWaiting()
{
    ...
    if (!m_pListnerImp)
    {
        log_debug("ERROR - CFtTcpListner::_onStartConnectionWaiting() m_pListnerImp is NULL (port:%i Name:\"%s\")",
            (int)m_nListenPort, CC(m_Name));
        return;
    }
...
void SocketLots::CLotsPool::FlushLots(SocketLots::eSendMethod SendMtd)
{
    ...
    // COMBINE ALL LOTS TO FINAL_FILE BUFFER
    {
        ar.beginChunk12(FTCHUNK_LOTS_BODYROOT, FTCHUNK_LOTS_BODYROOT_VER);
        {
            ...
            { // for All Lots
                ...
                {
                    if (vlog_socket >= 1)
                        log_warn("CLotsPool::FlushLots - CurLot is Locked! (%i) Socket:%X", pCurLot->GetNumLocks(),
                            ptr2DW(m_pSocket))
                            ->ASSERT_DLG();
                }
    ...
    try
    {
        ...
        if (vlog_socket >= 3)
            log_warn("CLotsPool::FlushLots() - Socket:%X nLotsCount:(%u) size:%u", ptr2DW(m_pSocket),
                (uint32_t)nLotsCount, (uint32_t)pMD->getSize());
    ...
    catch (std::exception& e)
    {
        ...
        log_debug("Socket:%X Pool Write EXCEPTION - Can't Write to socket! MSG=\"%s\"", ptr2DW(m_pSocket),
            CC(e.what()));
    }
}
...
// RAW DATA PROCESSOR
ETrisult DaSocket::CBaseDataProc::OnDataBlockReceived(/*Inout*/ DaSocket::TBlockPtr& pInOutBuf)
{
    if (pInOutBuf->isEmpty())
    {
        assert(0);
        return ETrisult::FAIL;
    }
...
// 	m_DebugDelay.reStart();
    ...
    if (vlog_socket >= 3)
        log_debug("CFTSocket:%X PopReadedMemFile() %u bytes bufQue:%u", ptr2DW(m_pSocket), pCmdFile->getSize(),
            (uint32_t)m_pOutReadBuffer.size());
