...
class SocketBaseCmd : public cmds::CBaseComand
{
    typedef cmds::CBaseComand super;
...
public:
    const ref_ptr<CSocketLocal>& getSocket() const { return m_pSocket; }
...
public:
    ...
    virtual EFlow update(qd::Fixed32 /*Delta*/) override
    {
        ...
        {
            log_debug("Socket:%X LocalSocket:%X HSocketSend SendSocketBuffer - Socket not Connected", ptr2DW(m_pSocket),
                ptr2DW(_hSocket()));
            return EFlow::STOP;
        }
        ...
        {
            if (vlog_socket >= 1)
                log_info("Socket:%X LocalSocket:%X HSocketSend Error: - Close connection", ptr2DW(m_pSocket),
                    ptr2DW(_hSocket()));
        ...
        if (vlog_socket >= 2)
            log_info("Socket:%X LocalSocket:%X HSocketSend - Send bytes: %u from %u", ptr2DW(m_pSocket),
                ptr2DW(_hSocket()), (uint32_t)Sent, nFullLen);
...
public:
    ...
    virtual EFlow update(qd::Fixed32 /*Delta*/) override
    {
        ...
        {
            if (vlog_socket >= 1)
                log_debug("Socket:%X LocalSocket:%X LocalReadSocket Command found - nullptr READ Buffer - STOP",
                    ptr2DW(m_pSocket), ptr2DW(_hSocket()));
        ...
        if (nRecived < 0)
        {
            ...
            if (vlog_socket >= 1)
                log_debug(
                    "Socket:%X LocalSocket:%X - Socket Read not connected while SocketRecv. SetConnectState(CLOSED)",
                    ptr2DW(m_pSocket), ptr2DW(_hSocket()));
            return EFlow::STOP;
        }
        ...
        if (nRecived == 0)
        {
            ...
            {
                if (vlog_socket >= 1)
                    log_debug("Socket:%X LocalSocket:%X LocalReadSocket Command found - Disconnected Socket - STOP",
                        ptr2DW(m_pSocket), ptr2DW(_hSocket()));
                return EFlow::STOP;
            }
            return EFlow::REPEAT;
        ...
        if (vlog_socket >= 2)
            log_debug("Socket:%X LocalSocket:%X LocalReadSocket Command Read received: %u bytes", ptr2DW(m_pSocket),
                ptr2DW(_hSocket()), (uint32_t)nRecived);
        ...
        cmds::CCommandList* pReadList = (cmds::CCommandList*)GetParent();
...
public:
    ...
    inline qd::ETrisult GetResult() const { return m_Result; }
...
public:
    ...
    const ref_ptr<Cmd::FlushSocketFuture_t>& GetFuture() const { return m_pFuture; }
...
cmds::CBaseComand* CSocketServiceLocal::AddReadCmd(Cmd::ReadSocketBuffer* pCmd)
{
    m_CmdMutex.lock();
...
cmds::CBaseComand* CSocketServiceLocal::AddSendCmd(Cmd::SendSocketBuffer* pCmd)
{
    m_CmdMutex.lock();
...
void CSocketServiceLocal::startAsync()
{
    assert(!m_pThread);
    assert(!m_pReadCommands);
    m_pReadCommands = new cmds::CCommandList();
    m_pSendCommands = new cmds::CCommandSequence();
...
{
    if (!IsConnected())
        throw CSocketException(CSocketException::CONNECTION_LOST, "Socket:%X is closed - can't Receive Data",
            ptr2DW(this));
...
{
    if (!IsConnected())
        throw CSocketException(CSocketException::CONNECTION_LOST, "Socket:%X is closed - can't Send Data",
            ptr2DW(this));
...
/*static*/
void _CancelAsyncSocketCommands(cmds::CCommandList* pCmdList, CFTSocket* pSocket)
{
    if (!pCmdList)
        return;
    ...
    log_info("Socket:%X LocalSocket:%X - LocalSocket.CancelAsyncSocketCommand - NumCommands:%i", ptr2DW(pMSocket),
        ptr2DW(_hSocket), numCommands);
...
cmds::CBaseComand* cmds::CCommandList::addCmd(const ref_ptr<cmds::CBaseComand>& pCommand)
{
    assert(pCommand);
