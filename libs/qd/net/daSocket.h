...
struct CInetAddr {
    ...
    CInetAddr(const qd::string& Host, uint16_t Port) { Set(Host, Port); }
    ...
    CInetAddr(const qd::string& HostWithPort) { parseHostWithPort(HostWithPort); }

    inline qd::string ToStringFull() const { return qd::string_format("%s:%u", CC(m_Host), (uint32_t)m_Port); }

    bool isValid() const { return m_Host.size() && m_Port; }

    void parseHostWithPort(const qd::string& HostWithPort);

    bool operator== (const CInetAddr& r) const
    {
        return m_Port == r.m_Port && qd::stricmp(m_Host.c_str(), r.m_Host.c_str()) == 0;
    }
...
protected:
    ...
    virtual void stopImp() { m_bDone = true; }
...
public:
    ...
    virtual CListnerImpBase* CreateSocketListner(CFtTcpListner* /*pListner */) { return nullptr; }

    virtual ref_ptr<CFTSocket> CreateSocket(DaSocket::CBaseDataProc* /*pDataProc*/) { return nullptr; }
...
public:
    ...
    virtual uint32_t RecommendBufferCapacity() { return 128; }

    int GetNumLocks() { return (m_nLocked); }

    bool IsLocked() { return (m_nLocked > 0); }
...
public:
    ...
    qd::MemFile* GetMemFile() const { return m_pMemFile; }
...
public:
    ...
    inline qd::CArchive& GetArchive() { return this->ar; }
...
public:
    ...
    ~CLotsPool() { assert(m_pLots.empty()); }

    int GetNumLots() const { return (int)m_pLots.size(); }

    bool IsEmpty() { return m_pLots.empty(); }
    ...
    void FlushSync() { FlushLots(SocketLots::SYNC); }
    void FlushAsync() { FlushLots(SocketLots::ASYNC); }
...
public:
    ...
    virtual ~CBaseDataProc() { m_pOutReadBuffer.clear(); }
...
struct CRawDataProc : public CBaseDataProc {
    ...
    virtual ETrisult OnDataBlockReceived(/*_Inout_*/ DaSocket::TBlockPtr& pReadBuf) override
    {
...
struct AppSlotDataProc : public CBaseDataProc {
    typedef CBaseDataProc super;
    friend class qd::CFTSocket;
    typedef AppSlotDataProc TThis;
...
public:
    ...
    virtual void HeaderData_End(qd::CArchive& ar) override { ar.endChunk(); }

}; // struct AppSlotDataProc
//////////////////////////////////////////////////////////////////////////
...
struct LotScope_ {
    ...
    TLot* operator->() const { return m_pLot.get(); }
...
class CFTSocket : public qd::RefCounted
{
    ...
    friend struct DaSocket::AppSlotDataProc;
...
public:
    ...
    inline CFTSocket::eConSt GetConnectState() const { return (CFTSocket::eConSt)(uint32_t)m_ConnectState; }

    bool IsInit() const { return m_bInit; }

    virtual void OnSocketConnected();

    virtual bool ConnectSync(const qd::string& /*pServerHoset*/, unsigned short /*Port*/) { return false; }
    virtual void Close();
    ...
    {
        assert(pFile && pFile->getBuffer());
        assert(pFile->getNumChunks() == 0);
    ...
    SocketLots::CLotsPool* GetLots() const { return m_pLotsPool; }
    ...
    virtual void FlushSocketSync() { FlushSync(); }

    void SetConnectName(const qd::string& ConnectName) { m_ConnectName = ConnectName; }
    const qd::string& GetConnectName() { return m_ConnectName; }

    DaSocket::CBaseDataProc* GetDataProc() const { return m_pDataProc; }
    DaSocket::AppSlotDataProc* GetSlotProc() const { return static_cast<DaSocket::AppSlotDataProc*>(m_pDataProc); }

    void SetDataProc(DaSocket::CBaseDataProc* DataProc) { m_pDataProc = DataProc; }

    const CInetAddr& GetConnectAddr() const { return m_ConnectAddr; }
    void SetConnectAddr(const CInetAddr& ConnectAddr) { m_ConnectAddr = ConnectAddr; }
...
public:
    ...
    inline DaSocket::LotScope_<TLot> MakeLot_(TArgs&&... args)
    { // SocketLots::CBaseLot*
    ...
    virtual qd::string GetEndPointIP() { return "UNKNOWN"; }
...
public:
    ...
    ref_ptr<MemData> PopReadedMemFile() { return m_pDataProc->PopReadedMemFile(); }
...
public:
    ...
    unsigned short GetPort() const { return m_nListenPort; }

    CSocketService* GetIOService() const { return m_pIOService; }
    template<class T>
    ...
    virtual ~CListnerImpBase() { m_pIOService = nullptr; }
...
public:
    CFtTcpListner(CSocketService* pIOService, unsigned short nPort, const CFtTcpListner::TOnConnectCB& OnConnect,
        qd::string Name);
    ~CFtTcpListner();
