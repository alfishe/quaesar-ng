...
protected:
    ...
    virtual uint32_t _arGetNumOpenChunks() override final { return (uint32_t)m_ChunkStack.size(); }

    virtual void _arReset() override final { m_ChunkStack.clear(); }
...
public:
    ...
    CMemoryArchiveBin(const ref_ptr<qd::MemBuf>& pMemBuffer, uint32_t nSize,
        ESaveLoad bStore /*= ESaveLoad::Load*/); // explicit does'nt work here

    explicit CMemoryArchiveBin(uint32_t outBuffer /* = 512*/, ESaveLoad bStore /*= ESaveLoad::Save*/);

    void reset();

    virtual ~CMemoryArchiveBin() override {
    ...
    qd::MemFile* GetMemTmpFile() const { return m_pMemTmpFile; }
