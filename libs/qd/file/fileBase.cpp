...
uint32_t StdFile::getSize() {
    ...
    if (!m_pFileHnd) {
        assert2(0, "StdIOFile GetSize File is Closed: %s", CC(getFileName()));
        return 0;
    }
...
uint32_t StdFile::read(void* pDest, uint32_t nBytes) {
    ...
    if (nReaded == 0) {
        ...
        if (pErr)
            throw Exception(EException::IO_ERROR, "StdFile::SDL_RWread - ERROR! NeedRead:%u Readed:%u err:'%s'", nBytes, nReaded, pErr);
    }
    m_curPos += nReaded;
    return nReaded;
}
...
uint32_t StdFile::write(const void* pSrc, uint32_t nBytes) {
    ...
    if (nWrited != nBytes) {
        const char* pErr = SDL_GetError();
        throw Exception(EException::IO_ERROR, "StdFile::Write - ERROR! NeedWrite:%u Writed:%u err:%s", nBytes, nWrited, pErr);
    }
    m_curPos += nWrited;
    m_nFileSize += nWrited;
    return nWrited;
