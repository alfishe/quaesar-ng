...
void CArchiveBin::_arLoadChunkBegin(const qd::IFileChunk& inChunk, qd::IFileChunk* pOutChunk) {
	...
	if (tp.m_LimitType == Arc::EChunkLim_U32 || tp.m_LimitType == Arc::EChunkLim_Unlim) {
		...
		assert(0 && "NOT IMPLEMENTED");
		throw Exception(EException::OPERATION_ERR, "Unknown Chunk Type size:%u", (uint32_t)inChunk.m_ChunkInfo.size_of());
	}
	if (pOutChunk)
...
void CArchiveBin::_arSaveChunkEnd() {
	...
	if (m_pFile->addNumChunks(-1) < 0) { // CHECK NUMBERS OF FILE CHUNKS
	...
	if (tp.m_LimitType == Arc::EChunkLim_U32 || tp.m_LimitType == Arc::EChunkLim_Unlim) {
		...
		if (bodySize32 > 0xFFFFFF) {
            log_error("FAILED: To save Tiny File ChunkID: '0x%4.X' ChunkSize=%u ", ChunkItem.m_FileChunk.getId(), bodySize32)->ASSERT_DLG();
			bodySize32 = 0xFFFFFF;
		}
		...
		if (bodySize32 > 0xFFFF) {
            log_error("FAILED: Chunk4 content is too long to save. ChunkID: '0x%4.X' ChunkSize=%u ", ChunkItem.m_FileChunk.getId(), bodySize32)->ASSERT_DLG();
			bodySize32 = 0xFFFF;
		}
		...
		if (bodySize32 > 0xFF) {
			log_error("FAILED: Chunk4 content is too long to save. ChunkID: '0x%4.X' ChunkSize=%u ", ChunkItem.m_FileChunk.getId(), bodySize32)
				->ASSERT_DLG();
			bodySize32 = 0xFF;
		}
...
void CArchiveBin::_arWrite_Buf(const void* pSrc, uint32_t nBytes) {
	...
	if (nBytesWrited != nBytes)
		throw Exception(EException::IO_ERROR, "Serialization ERROR: Cannot write %d bytes, %d written", nBytes, nBytesWrited);
}
...
void CArchiveBin::_arRead_Buf(void* pDest, uint32_t nBytes) {
	...
	if (nBytesReaded != nBytes)
		throw Exception(EException::IO_ERROR, "Serialization ERROR: Cannot read %d bytes, %d read", nBytes, nBytesReaded);
}
...
// 	CArchive::CArchive( const CArchive& Other ) {
// 		(*this) = Other;
// 	}
...
void CArchive::beginChunk(_Inout_ qd::IFileChunk& Chunk) {
	if (isStoring()) {
...
void IFileChunk::rewind(CArchive& ar) const {
	...
	ar.LoadChunk_(loadedCh, true);
  	if (getId() != loadedCh.getId())
  		throw Exception(EException::IO_ERROR, "Wrong Chunk ID. Expected '0x%4.X', Receive: '0x%4.X'", (uint32_t)loadedCh.getId(), (uint32_t)getId());
}
...
IFileChunk::IFileChunk(qd::CArchive& ar, uint32_t ID, qd::Arc::EChunkType _chunkType)
	...
	if (ar.isLoading()) {
		...
		if (m_ID != ID) {
			...
			throw Exception(EException::IO_ERROR, "Wrong Chunk ID. Expected '0x%4.X', Receive: '0x%4.X'", (uint32_t)ID, (uint32_t)m_ID);
		}
	}
}
...
void IFileChunkScope::endChunk()
{
	...
	if (!bOk)
	{
		...
		{
            throw Exception(EException::IO_ERROR, "WARNING: Chunk%u ID:0x%4X not read fully!", (uint32_t)m_ChunkInfo.m_SizeOf, m_ID);
				//->THROW_ASSERT();
		}
...
void IFileChunk::_serializeChunkID32(qd::CArchive& ar, uint32_t ID, uint32_t nVersion /*= 0*/, bool bCheckId /*= true*/) {
	...
	if (ar.isStoring()) {
		...
		if (bCheckId && (m_ID != ID) && m_ChunkInfo.m_IDType != Arc::EChunkID_U0) {
			m_bWasException = true;
			throw Exception(EException::IO_ERROR, "Wrong Chunk ID Loaded. Expected '0x%4.X', Receive: '0x%4.X'", (uint32_t)ID, (uint32_t)m_ID);
		}
	}
}
...
// 		m_bWasException = true;
// 		throw qd::Exception(qd::EException::IO_ERROR, "Wrong Chunk ID Loaded. Expected '%s', Receive: '%s'", CC(pStrID), CC(m_pIDStr));
// 	}
}
...
bool CArchive::loadChunk(IFileChunk& outFileChunk, uint32_t checkChunkID /*= 0*/, bool bThrow /*= true*/) {
	...
	if ( origFileChunk.m_ID != 0 && origFileChunk.m_ID != outFileChunk.m_ID ) {
		outFileChunk.m_bWasException = true;
		if (bThrow)
			throw Exception(EException::IO_ERROR, "Wrong Chunk ID Loaded. Expected '0x%4.X', Receive: '0x%4.X'", checkChunkID, outFileChunk.m_ID);
		return false;
	}
	return true;
}
