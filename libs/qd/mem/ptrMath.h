...
template<class T, typename TInt>
[[nodiscard]] constexpr inline uint8_t* ptrAdd(const T* ptr, TInt offset) {
    uint8_t* p8 = (uint8_t*)ptr;
    p8 += offset;
    return p8;
}
...
[[nodiscard]] constexpr inline TResPtr* ptrAdd_(T ptr, TInt offset) {
...
[[nodiscard]] constexpr inline size_t ptrDiff(TA ptrA, TB ptrB) {
    const size_t diff = (const uint8_t*)ptrA - (const uint8_t*)ptrB;
    return diff;
}

template<class T>
[[nodiscard]] constexpr inline uint32_t ptr2DW(T pPtr) {
    return static_cast<uint32_t>((std::size_t)((void*)pPtr)); //-V205 //-V221
}

template<class T>
[[nodiscard]] constexpr inline void* DW2Ptr(const T& pPtr) {
