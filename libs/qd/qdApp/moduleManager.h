...
#define QD_MODULE_REGISTRATION(ClassName)	\
	static qd::CModuleRegistrator_<ClassName> PASTE(gModuleImp_,__COUNTER__ ) ( /**/STRINGIFY(ClassName) );
#define QD_MODULE_REGISTRATION_NOCREATE(ClassName)	\
	static qd::CModuleRegistratorNoCreate_<ClassName> PASTE(gModuleImp_,__COUNTER__ ) ( /**/STRINGIFY(ClassName) );
...
{
public:static qd::ECgModuleID getModuleTypeId() { return qd::ECgModuleID::DUMMY; } private:
...
static CModuleRegistrator_<CDummyModule> s;
...
public:
	...
	CModuleInfo( ECgModuleID ModuleID )
		: m_ModuleID( ModuleID )
		, m_pInstance(nullptr)
		, m_nInstanceRef(0)
	{
	...
	qd::ECgModuleID getModuleId() const {
		return m_ModuleID;
	}

	void setModuleId(qd::ECgModuleID ModuleID) {
		m_ModuleID = ModuleID;
	}

	const ref_ptr<IModuleInterface>& getInstance() const {
		return m_pInstance;
	}
    ...
    void merge(const CModuleInfo& r);
...
struct CModuleRegistrator_
{
	typedef CModuleRegistrator_<TModule> TThis;
	inline CModuleRegistrator_( const char* pClassName ); // CONSTRUCTOR TO REGISTER THIS STRUCT IN #ModuleManager
...
template<class TModule>
struct CModuleRegistratorNoCreate_
{
	typedef CModuleRegistratorNoCreate_<TModule> TThis;
	inline CModuleRegistratorNoCreate_( const char* pClassName ); // CONSTRUCTOR TO REGISTER THIS STRUCT IN #ModuleManager
...
class ModuleManager
{
	...
	eastl::fixed_map< uint32_t, eastl::unique_ptr<CModuleInfo>, g_nMAX_MODULES, false > m_pModuleInfoMap;
public: typedef eastl::fixed_map< uint32_t, eastl::unique_ptr<CModuleInfo>, g_nMAX_MODULES, false > TModuleInfoMap;

	 eastl::fixed_vector< CModuleInfo*, ECgModuleID::_FAST_ACCESS_, false > m_pFastModules;

private: static TThis* m_pSingleInstance;
private: static bool m_bSingleDestroyed;
public:
...
public:
	...
	const ModuleManager::TModuleInfoMap& getModuleInfoMap() const {
		return m_pModuleInfoMap;
	}


	CModuleInfo* overrideModule(ECgModuleID ModuleId, const CModuleInfo::TCreateFunc& pCreateFunc);


	// Default create function - concreate module
	template<class TModuleClass>
	inline CModuleInfo* overrideModule_( const CModuleInfo::TCreateFunc& pCreateFunc = CCallbackNull() );


	CModuleInfo* registerModule(const CModuleInfo& i);


	CModuleInfo* findModuleInfo(ECgModuleID ModuleId) const;

	CModuleInfo* getOrCreateModuleInfo(ECgModuleID ModuleId);
	...
	qd::IModuleInterface* createModuleInstance( ECgModuleID ModuleId, bool bSingletonInstance, qd::ModuleCreateParams* pCreateParam = nullptr );
...
inline TModule* getModuleInst_() {
	ptr<TModule> pExistInst = ModuleManager::get()->getModuleInstance(TModule::getModuleTypeId(), false);
	return pExistInst;
}
...
template<class TModuleClass>
inline CModuleInfo* ModuleManager::overrideModule_( const CModuleInfo::TCreateFunc& pCreateFunc /*= CCallbackNull() */ )
{
	ECgModuleID ModuleId = TModuleClass::getModuleTypeId();
	if ( pCreateFunc ) {
		// CUSTOM CREATE FUNC
		...
		// DEFAULT CREATE FUNC
		return overrideModule( ModuleId, BIND_FREE_CB( &CModuleRegistrator_<TModuleClass>::createModuleFunc ) );
	}
}
...
template<class TModuleClass>
qd::IModuleInterface* CModuleRegistrator_<TModuleClass>::createModuleFunc(qd::ModuleCreateParams* pCP)
{
...
template<class TModule>
inline CModuleRegistrator_<TModule>::CModuleRegistrator_( const char* pClassName ) {
	ECgModuleID ModuleId = TModule::getModuleTypeId();
	CModuleInfo* pMtd = ModuleManager::I()->overrideModule( ModuleId, BIND_FREE_CB( &TThis::createModuleFunc ) );
	pMtd->m_ModuleName = pClassName;
}


template<class TModule>
inline CModuleRegistratorNoCreate_<TModule>::CModuleRegistratorNoCreate_( const char* pClassName ) {
	ECgModuleID ModuleId = TModule::getModuleTypeId();
	CModuleInfo* pMtd = ModuleManager::I()->overrideModule( ModuleId, BIND_FREE_CB( &TThis::CreateModuleFunc ) ); // NO CREATE
	pMtd->m_ModuleName = pClassName;
}


template<class TModuleClass>
TModuleClass* CModuleInfo::makeInstance_(bool bRegisterSingleton, qd::ModuleCreateParams* mc /*= nullptr */)
{
