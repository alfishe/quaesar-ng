...
#endif
// clang-format off
// windows.h must be included before dbghelp.h for PCONTEXT type
#include <windows.h>
#include <dbghelp.h>
#include <strsafe.h>
// clang-format on
...
static void writeStackTrace(EXCEPTION_POINTERS* ep) {
    ...
    char line[1024];
    int frameNo = 0;
    while (StackWalk64(machine, process, thread, &frame, &ctx, nullptr, SymFunctionTableAccess64, SymGetModuleBase64,
                       nullptr)) {
        ...
        if (SymFromAddr(process, pc, &symDisp, sym)) {
            ...
            if (SymGetLineFromAddr64(process, pc, &lineDisp, &lineInfo)) {
                StringCchPrintfA(line, sizeof(line), "  [%2d] 0x%016llx  %s   at %s:%lu\n", frameNo,
                                 (unsigned long long)pc, sym->Name, lineInfo.FileName, lineInfo.LineNumber);
            } else {
...
static void writeRegisters(EXCEPTION_POINTERS* ep) {
    ...
    StringCchPrintfA(buf, sizeof(buf),
                     ...
                     (unsigned long long)c.Rbp, (unsigned long long)c.Rsp, (unsigned long long)c.R8,
                     (unsigned long long)c.R9, (unsigned long long)c.R10, (unsigned long long)c.R11,
                     (unsigned long long)c.R12, (unsigned long long)c.R13, (unsigned long long)c.R14,
                     (unsigned long long)c.R15, (unsigned long long)c.Rip, (unsigned long)c.EFlags);
#elif defined(_M_IX86)
    StringCchPrintfA(buf, sizeof(buf),
                     "\nRegisters:\n"
    ...
    StringCchPrintfA(buf, sizeof(buf),
                     ...
                     (unsigned long long)c.Pc, (unsigned long long)c.Sp, (unsigned long long)c.Fp,
...
static LONG WINAPI sehFilter(EXCEPTION_POINTERS* ep) {
    ...
    StringCchPrintfA(dumpPath, MAX_PATH, "%squaesar_crash_%lu_%lu.dmp", tempDir, pid, GetTickCount());
    ...
    if (hFile != INVALID_HANDLE_VALUE) {
        ...
        MiniDumpWriteDump(GetCurrentProcess(), pid, hFile,
                          static_cast<MINIDUMP_TYPE>(MiniDumpNormal | MiniDumpWithThreadInfo), &mei, nullptr, nullptr);
        CloseHandle(hFile);
