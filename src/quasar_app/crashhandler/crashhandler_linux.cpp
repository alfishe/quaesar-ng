...
static void writeErr(const char* msg) {
    ssize_t n = write(STDERR_FILENO, msg, strlen(msg)); (void)n;
}
...
static void writeRegisters(ucontext_t* uc, char* buf, size_t bufSize) {
    ...
    snprintf(buf, bufSize,
             ...
             "  EIP=%08x  EFLAGS=%08x\n",
             (unsigned)mc.gregs[REG_EAX], (unsigned)mc.gregs[REG_EBX], (unsigned)mc.gregs[REG_ECX],
             (unsigned)mc.gregs[REG_EDX], (unsigned)mc.gregs[REG_ESI], (unsigned)mc.gregs[REG_EDI],
             (unsigned)mc.gregs[REG_EBP], (unsigned)mc.gregs[REG_ESP], (unsigned)mc.gregs[REG_EIP],
             (unsigned)mc.gregs[REG_EFL]);
    ...
    snprintf(buf, bufSize,
             ...
             "  PC =%016llx\n",
             (unsigned long long)mc.regs[0], (unsigned long long)mc.regs[1], (unsigned long long)mc.regs[2],
             (unsigned long long)mc.regs[3], (unsigned long long)mc.regs[4], (unsigned long long)mc.regs[5],
             (unsigned long long)mc.regs[6], (unsigned long long)mc.regs[7], (unsigned long long)mc.regs[8],
             (unsigned long long)mc.regs[9], (unsigned long long)mc.regs[10], (unsigned long long)mc.regs[11],
             (unsigned long long)mc.regs[12], (unsigned long long)mc.regs[13], (unsigned long long)mc.regs[14],
             (unsigned long long)mc.regs[15], (unsigned long long)mc.regs[16], (unsigned long long)mc.regs[17],
             (unsigned long long)mc.regs[18], (unsigned long long)mc.regs[19], (unsigned long long)mc.regs[20],
             (unsigned long long)mc.regs[21], (unsigned long long)mc.regs[22], (unsigned long long)mc.regs[23],
             (unsigned long long)mc.regs[24], (unsigned long long)mc.regs[25], (unsigned long long)mc.regs[26],
             (unsigned long long)mc.regs[27], (unsigned long long)mc.regs[28], (unsigned long long)mc.regs[29],
             (unsigned long long)mc.regs[30], (unsigned long long)mc.sp, (unsigned long long)mc.pc);
#else
...
static void posixSignalHandler(int sig, siginfo_t* info, void* context) {
    ...
    char logPath[64];
    snprintf(logPath, sizeof(logPath), "/tmp/quaesar_crash_%ld_%d.log", (long)now, getpid());

    FILE* f = fopen(logPath, "w");
    if (f) {
        fprintf(f, "*** UNHANDLED SIGNAL (Quaesar) ***\n");
